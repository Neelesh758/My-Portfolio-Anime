...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      toast.success(res.data.message || "Registeration successfull")
      // console.log(res)
  ...
  return (
    ...
    className='w-[350px] h-[400px] bg-black/70 backdrop-blur-md border border-red-500/30 rounded-2xl'>
      <div>
        <h1 className='text-red-500 font-bold text-center text-3xl mb-6'>Register</h1>
        <p className='text-gray-400 text-md mb-6 text-center'>Create your Account</p>
      </div>
      <div className='p-4'>
        <div className=' flex flex-col justify-between items-center'>
          {/* First Input */}
          <input type="text" value={userName} onChange={(e)=>setUserName(e.target.value)} placeholder='Enter User Name' className='w-full mb-4 px-4 py-3 rounded-lg bg-zinc-900 text-gray-100 placeholder-gray-500 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/50 transition' />
          {/* Second Input */}
        ...
        <button className='w-full mt-4 py-3 rounded-lg font-semibold bg-gradient-to-r from-red-600 to-red-800 text-white hover:from-red-500 hover:to-red-700 transition duration-300' onClick={handleRegister}>Submit</button>
      </div>
    </motion.div>
  )
}
